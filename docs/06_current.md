...
platform profile. Full detail: [`05a_codemap.md`](05a_codemap.md). **Committed** — folded into
`"Initial commit"` (`e9b7e09`) along with the `docs/` scaffold; `git status` is clean. (Corrected
2026-09-15 — earlier said uncommitted.)

## 2. Next steps

In order, per [`04_plan.md`](04_plan.md)§2:

1. Build the dedicated venv (`05b_remote.md`§3) — **cannot** reuse `~/dev/openvla/.venv`.
2. Push this repo to a remote checkout on `/cpfs01` (neither `devolremote` nor `gpu245` has one
   yet) and submit a LoRA fine-tune (`--use_l1_regression True --use_film False
   --num_images_in_input 1 --lora_rank 32 --use_proprio False`).
...
- ~~**`--use_proprio` is an open decision**~~ **Decided 2026-09-15: `False`** (`Ah_for_human.md`§1
  Q1); chunk size 8 also confirmed (Q2).
